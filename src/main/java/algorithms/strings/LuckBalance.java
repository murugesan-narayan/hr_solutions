...
public class LuckBalance {
    static int luckBalance(int k, int[][] contests) {
        ...
        impContests.sort(Comparator.<Integer>naturalOrder().reversed());
        int sum = 0;
        for (int i = 0; !impContests.isEmpty() && i < k; i++) {
            sum += impContests.remove(0);
        }
        int unImpCount = unImpContests.stream().reduce(Integer::sum).orElse(0);
        int remImpCount = impContests.stream().reduce(Integer::sum).orElse(0);
